...
const STEPS = [
  { key: "starting", label: "Analyzing", description: "Examining industry trends and location dynamics" },
  { key: "calling_ai", label: "Consulting AI", description: "Querying LLM models for real-world pain points" },
  { key: "parsing", label: "Processing", description: "Handling discovered problems and validating criteria" },
  { key: "complete", label: "Complete", description: "Discovery session finished successfully" },
];

export const StreamProgress: React.FC<StreamProgressProps> = ({ currentStep }) => {
  ...
  return (
    <div className="w-full max-w-xl mx-auto bg-slate-950/40 backdrop-blur-xl border border-white/5 rounded-3xl p-6 sm:p-8 shadow-2xl relative overflow-hidden">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h3 className="text-white font-bold text-lg">AI Discovery Pipeline</h3>
          <p className="text-slate-400 text-xs sm:text-sm">Please wait while the AI analyzes the industry</p>
        </div>
      </div>
